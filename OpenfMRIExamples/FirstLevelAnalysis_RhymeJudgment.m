...
T1_nii = load_nii([basepath study '/sub00' num2str(subject) '/anatomy/highres001_brain.nii.gz']);
...
EPI_nii = load_nii([basepath study '/sub00' num2str(subject) '/BOLD/task001_run001/bold.nii.gz']);
...
fid = fopen([basepath study '/sub00' num2str(subject) '/model/model001/onsets/task001_run001/cond001.txt']);
...
fid = fopen([basepath study '/sub00' num2str(subject) '/model/model001/onsets/task001_run001/cond002.txt']);
text = textscan(fid,'%f%f%f');
fclose(fid);
...
for i = 1:length(onsets1)    
    start = round(onsets1(i)*100000/TR); 
    activity_length = round(durations1(i)*100000/TR);
    value = values1(i);

    for j = 1:activity_length
       X_GLM_highres(start+j,1) = value; 
    end                    
end

for i = 1:length(onsets2)    
    start = round(onsets2(i)*100000/TR); 
    activity_length = round(durations2(i)*100000/TR);
    value = values2(i);

    for j = 1:activity_length
       X_GLM_highres(start+j,2) = value; 
    end                    
end

% Downsample
X_GLM_1 = decimate(X_GLM_highres(:,1),100000);
X_GLM_2 = decimate(X_GLM_highres(:,2),100000);
