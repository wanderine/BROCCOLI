...
for s = 1:N
    ...
    s
    mc = load_nii([basepath_AFNI '/AFNI_motion_corrected_subject'  num2str(s) '_random_motion.nii']);
    mc = double(mc.img);
    mc(isnan(mc)) = 0;
    mc = mc/max(mc(:));
    reference = mc(:,:,:,1);    
...
for s = 1:N
    ...
    s
    mc = load_nii([basepath_FSL '/FSL_motion_corrected_subject'  num2str(s) '_random_motion.nii.gz']);
    mc = double(mc.img);
    mc(isnan(mc)) = 0;
    mc = mc/max(mc(:));
    reference = mc(:,:,:,1);    
...
for s = 1:N
    ...
    mc1 = load_nii([basepath_none '/' subject]);
    mc1 = double(mc1.img);
    mc1(isnan(mc1)) = 0;
    mc1 = mc1 / max(mc1(:));    
    reference = mc1(:,:,:,1);    
