...
voxel_size_x = T1_nii.hdr.dime.pixdim(1);
voxel_size_y = T1_nii.hdr.dime.pixdim(2);
voxel_size_z = T1_nii.hdr.dime.pixdim(3);

if ( ((voxel_size_x - 1.00) > 0.05) || ((voxel_size_y - 1.00) > 0.05) || ((voxel_size_z - 1.00) > 0.05))
    % Calculate new number of elements
    sx_T1_new = round(sx_T1*voxel_size_x / 1.0);
    sy_T1_new = round(sy_T1*voxel_size_y / 1.0);
    sz_T1_new = round(sz_T1*voxel_size_z / 1.0);
