...
statistical_maps_cpu_no_whitening = zeros(sy,sx,sz,size(contrasts,1));
statistical_maps_cpu_co = zeros(sy,sx,sz,size(contrasts,1));
    ...
    for y = 1:sy
        for z = 1:sz
            if brain_mask(y,x,z) == 1
                ...
                for i = 1:size(contrasts,1)
                    contrast = contrasts(i,:)';
                    statistical_maps_cpu_no_whitening(y,x,z,i) = contrast'*beta / sqrt( residual_variances_cpu_no_whitening(y,x,z) * ctxtxc_GLM(i));
                end
    ...
    for y = 1:sy
        for z = 1:sz
            if brain_mask(y,x,z) == 1
                ...
                for i = 1:size(contrasts,1)
                    contrast = contrasts(i,:)';
                    GLM_scalar = contrast'*inv(whitened_X_GLM'*whitened_X_GLM)*contrast;
                    statistical_maps_cpu_co(y,x,z,i) = contrast'*beta / sqrt( residual_variances_cpu_co(y,x,z) * GLM_scalar);
                end
...
[betas_opencl, residuals_opencl, residual_variances_opencl, statistical_maps_opencl, ...
    ...
    GLMTTestFirstLevel(fMRI_volumes,brain_mask,smoothed_mask,X_GLM,xtxxt_GLM',contrasts,ctxtxc_GLM,EPI_smoothing_amount,AR_smoothing_amount,...
    EPI_voxel_size_x,EPI_voxel_size_y,EPI_voxel_size_z,opencl_platform,opencl_device);
toc


figure
imagesc([statistical_maps_cpu_no_whitening(:,:,20) statistical_maps_cpu_co(:,:,20)     ]); colorbar

figure
imagesc([ar1_estimates_cpu(:,:,20) ar1_estimates_cpu_co(:,:,20)      ]); colorbar
...
figure
imagesc([ statistical_maps_cpu_no_whitening(:,:,slice,1) statistical_maps_cpu_co(:,:,slice,1) statistical_maps_opencl(:,:,slice,1)]); colorbar
title('Statistical maps')

figure
imagesc([ statistical_maps_cpu_co(:,:,slice,1) - statistical_maps_opencl(:,:,slice,1)]); colorbar
title('Statistical map diff')

slice = 19;
threshold = 4;
figure
imagesc([ statistical_maps_cpu_no_whitening(:,:,slice,1) .* (statistical_maps_cpu_no_whitening(:,:,slice,1) > threshold ) statistical_maps_cpu_co(:,:,slice,1) .* (statistical_maps_cpu_co(:,:,slice,1) > threshold ) statistical_maps_opencl(:,:,slice,1) .* (statistical_maps_opencl(:,:,slice,1) > threshold ) ]); colorbar
title('Statistical maps thresholded')
